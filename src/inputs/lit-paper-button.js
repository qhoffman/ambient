...
export class litPaperButton extends LitElement {
  ...
  updated() {
    ...
    if (this.backgroundColor !== '') {
      this.shadowRoot.getElementById('button').style.backgroundColor =
    ...
    if (this.borderRadius !== '0px') {
      this.shadowRoot.getElementById('button').style.borderRadius =
        this.borderRadius || '5px';
    }
    if (this.padding !== '') {
      this.shadowRoot.getElementById('button').style.padding =
        this.padding || '10px';
    }
  }
  ...
  doThing(e) {
    ...
    const parent = this.shadowRoot.getElementById('button');
    const rip = document.createElement('div');
    rip.id = 'ripple';
    parent.appendChild(rip);
    const parentDims = parent.getBoundingClientRect();
    const max = Math.max(parentDims.width, parentDims.height);
    if (this.rippleColor) {
